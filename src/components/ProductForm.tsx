...
export default function ProductForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.refresh(); // Re-render the page to show the new product
    } catch (error: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 bg-gray-100 rounded-lg shadow-md max-w-lg mx-auto">
      ...
      <div className="mb-4">
        ...
        <input type="url" name="imageUrl" value={formData.imageUrl} onChange={handleChange} required className="w-full mt-1 p-2 border rounded-md" />
      </div>
